...
public class BoatAlignNormal : MonoBehaviour, IBoat
{
    ...
    float speed(float yMov)
    {
        if (yMov < 0)
        {
            return Math.Max(-1f,Math.Min(0f,-yMov * yMov / 5000f + 1f / 50f));
        } else // rightY > 0
        {
            return Math.Min(1f,Math.Max(0f,yMov * yMov / 5000f - 1f / 50f));
        }
    }
    ...
    void FixedUpdate()
    {
        ...
        */
        Vector3 leftEuler = LeftController.transform.eulerAngles;
        Vector3 rightEuler = RightController.transform.eulerAngles;

        float localLeftEulerY = LeftController.transform.localEulerAngles.y%360;
        float localRightEulerY = RightController.transform.localEulerAngles.y%360;

        float leftY = localLeftEulerY > 180 ? localLeftEulerY - 360 : localLeftEulerY;
        float rightY = localRightEulerY > 180 ? localRightEulerY - 360 : localRightEulerY;
        ...
        {
            if(leftEuler.x > 285 && rightEuler.x > 285) //moving forward
            {
                rawForward = (leftEuler.x-270)/90;
                forward += rawForward;
                RB.AddForceAtPosition(transform.forward * _enginePower * forward, forcePosition, ForceMode.Acceleration);
            }
            if(leftY > -100 && leftY < -10 && rightY > -100 && rightY < -10) //turning left   && rightEuler.y > 30 && rightEuler.y < 150
            {
                sideways += speed(leftY);
                //print(sideways);
                //(Input.GetKey(KeyCode.A) ? reverseMultiplier * -1f : 0f) +
                //(Input.GetKey(KeyCode.D) ? reverseMultiplier * 1f : 0f);
                RB.AddTorque(transform.up * _turnPower * sideways, ForceMode.Acceleration);
            }
            if(leftY > 10 && leftY < 100 && rightY > 10 && rightY < 100) //turning right   rightEuler.y > 0 && rightEuler.y < 50 && 
            {
                sideways += speed(rightY);
                RB.AddTorque(transform.up * _turnPower * sideways, ForceMode.Acceleration);
            }
